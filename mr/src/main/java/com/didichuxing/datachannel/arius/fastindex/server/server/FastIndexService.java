...
import com.didichuxing.datachannel.arius.fastindex.server.common.es.catshard.item.ESNode;
import com.didichuxing.datachannel.arius.fastindex.server.common.es.catshard.item.ESShard;
import com.didichuxing.datachannel.arius.fastindex.server.common.es.mapping.MappingConfig;
import com.didichuxing.datachannel.arius.fastindex.server.common.es.mapping.TypeConfig;
import com.didichuxing.datachannel.arius.fastindex.server.common.es.mapping.TypeDefine;
import com.didichuxing.datachannel.arius.fastindex.server.common.po.*;
import com.didichuxing.datachannel.arius.fastindex.server.dao.*;
import com.didichuxing.datachannel.arius.fastindex.server.utils.SizeUtil;
...
public class FastIndexService {
     ...
     *
     */
    public static IndexFastIndexInfo getIndexConfig(String template, long time, long hdfsSize) throws Exception {
     ...
     * @param srcTag
     */
    public static void startLoadData(String template, long time, String hdfsDir, int expanFactor, String hdfsUser, String hdfsPasswd, String srcTag) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     */
    public static void submitMetric(String srcTag, String template, long time, long reduceId, JSONObject metric) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     * @param time 时间分区
     */
    public static JSONObject getAllMetrics(String srcTag, String template, long time) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     */
    public static void sumitMapping(String srcTag, String template, long time, long reduceId, JSONObject mapping) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     * @param time 时间分区
     */
    public static MappingConfig mergeMapping(String srcTag, String template, long time) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     * @param time 分区时间
     */
    public static void removeFinishTag(String srcTag, String template, long time) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     * @param time 分区时间
     */
    public static boolean isFinish(String srcTag, String template, long time) throws Exception {
        String indexName = getIndexName(template, time);
     ...
     * @param time 分区时间
     */
    private static boolean isFinish(String srcTag, String indexName) {
    ...
    /* 去除路径末尾的/ */
    private static String getHdfsPath(String rootPath) {
    ...
    private static String getEsPathFor661(String uuid, long shardId) {
    ...
    private static String getIndexName(String template, long time) {
        SimpleDateFormat sdf = new SimpleDateFormat(template+DATE_FORMAT);
        return sdf.format(time);
    }

    /* 获得索引配置 */
    private static IndexConfig getIndexConfig(String indexName) throws Exception {
    ...
    /* 对比srcTag */
    private static boolean equalTag(String tag1, String tag2) {
        if (tag1 == null) {
            if (tag2 == null) {
                return true;
            } else {
    ...
    public static int decideExpanFactor(long shardNum, long hdfsSize) {
        if(hdfsSize<=0) {
            return EXPANFACTOR;
        }
